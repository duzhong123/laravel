...
@section('content')
    <div class="maincont">
     ...
     <table class="shoucangtab">
      <tr>
       <td width="75%"><span class="hui">购物车共有：{{$res}}<strong class="orange"></strong>件商品</span></td>
     ...
     <div class="dingdanlist">
      <table>
       ...
       <tr goods_id="{{$v->goods_id}}">
        ...
        <td class="dingimg" width="15%"><img src="{{env('IMGS_URL')}}{{$v->goods_img}}"/></td>
        <td width="50%">
         <h3>{{$v->goods_name}}</h3>
         <time>下单时间:@php echo date('Y-m-d H:i:s',$v->addtime)@endphp</time>
        </td>
        <td align="right" goods_num="{{$v->goods_num}}">
        <button class="decreasesss" >-</button>
        <input type="text" value="{{$v->buy_number}}"  class="spinnerExample" />
